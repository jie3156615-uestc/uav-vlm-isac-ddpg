...
class GateMechanism(nn.Module):
    ...
    def forward(self, vlm_feat, isac_feat):
        ...
        fused = gate * vlm_feat + (1 - gate) * isac_feat
        return fused, gate
...
class DDPGEnvironment(gym.Env):
    ...
    def __init__(
        ...
        self.initial_battery = self.e_battery_uav
        ...
        if self.large_paper8_mode:
            ...
            if position_key == "large_paper8":
                ...
                self.large_paper_isac_heading_assist_weight = 0.80
                self.large_paper_vlm_heading_assist_weight = 0.45
                # High-rise VLM-only links are less reliable without ISAC-grade alignment.
        ...
        self.train_gate_debug = False

        self.index = np.ones(self.num_ground_stations)
        self.index_tsp_isac = np.ones(self.num_ground_stations)
        self.index_tsp_vlm = np.ones(self.num_ground_stations)
    ...
    def _sample_gt_motion(self, loc_gt):
        walkspeed = random.uniform(0, 4)
        for gs in loc_gt:
            alpha = random.uniform(0, 2) * np.pi
            next_x = gs[0] + walkspeed * self.t * math.cos(alpha)
            next_y = gs[1] + walkspeed * self.t * math.sin(alpha)
    ...
    def _estimate_distances(self, loc_uav_array, loc_gt_array, mode):
        ...
        if mode == 2:
            return true_distances, no_isac_est, no_isac_bearings, {}, None, None
        if mode == 1:
            return true_distances, vlm_est, vlm_bearings, {}, None, None
        if mode == 0:
            return true_distances, isac_est, isac_bearings, {}, None, None

        vlm_tensor = self._distance_tensor(vlm_est)
        isac_tensor = self._distance_tensor(isac_est)
        fused_tensor, gate_weights = fuse_vlm_isac(vlm_tensor, isac_tensor, self.gate_module)
        fused_est = np.maximum(fused_tensor.detach().cpu().numpy().reshape(-1) * self.distance_scale, 1e-3)
        gate_np = gate_weights.detach().cpu().numpy().reshape(-1)
        fused_bearings = gate_np * vlm_bearings + (1.0 - gate_np) * isac_bearings
        fusion_debug = {
            "true": true_distances.copy(),
            "fused": fused_est.copy(),
            "g": gate_np,
            "vlm": vlm_est.copy(),
            "isac": isac_est.copy(),
        }
        return true_distances, fused_est, fused_bearings, fusion_debug, fused_tensor, gate_weights
    ...
    def _build_aux_features(self, loc_uav, true_distances, sensing_distances, los_probs, observed_positions, true_positions, guide_strength):
        sensing_norm = np.clip(sensing_distances / self.distance_scale, 0.0, 1.0)
        true_norm = np.clip(true_distances / self.distance_scale, 0.0, 1.0)
        error_norm = np.clip(np.abs(sensing_distances - true_distances) / self.distance_scale, 0.0, 1.0)
        ...
        centroid_rel = (centroid_xy - np.asarray(loc_uav[:2], dtype=float)) / self.distance_scale
        nearest_xy = observed_positions[int(np.argmin(sensing_distances)), :2]
        nearest_rel = (nearest_xy - np.asarray(loc_uav[:2], dtype=float)) / self.distance_scale
        true_centroid_xy = np.mean(true_positions[:, :2], axis=0)
        true_centroid_rel = guide_strength * (true_centroid_xy - np.asarray(loc_uav[:2], dtype=float)) / self.distance_scale
        true_nearest_xy = true_positions[int(np.argmin(true_distances)), :2]
        true_nearest_rel = guide_strength * (true_nearest_xy - np.asarray(loc_uav[:2], dtype=float)) / self.distance_scale
        base_aux = np.array(
            [
                np.min(sensing_norm),
                np.mean(sensing_norm),
                np.max(sensing_norm),
                np.std(sensing_norm),
                nearest_idx,
                np.min(los_probs),
        ...
        true_rel = guide_strength * (true_positions[:, :2] - np.asarray(loc_uav[:2], dtype=float)) / self.distance_scale
    ...
    def _get_state(self, loc_uav, ground_stations, true_distances=None, sensing_distances=None, sensing_bearings=None, los_probs=None, mode=0):
        uav_normalized = np.array(
            [loc_uav[0] / self.ground_length, loc_uav[1] / self.ground_width, loc_uav[2] / self.height_max],
            dtype=float,
        )
        if true_distances is None:
            loc_uav_array = np.asarray(loc_uav, dtype=float)
            loc_gt_array = np.asarray(ground_stations, dtype=float)
            true_distances = np.linalg.norm(loc_uav_array[:2] - loc_gt_array[:, :2], axis=1)
        if sensing_distances is None:
            sensing_distances = true_distances
        if sensing_bearings is None:
            sensing_bearings = np.arctan2(ground_stations[:, 1] - loc_uav[1], ground_stations[:, 0] - loc_uav[0])
        if los_probs is None:
            los_probs = np.array([self.get_los_probability(loc_uav[2], d) for d in true_distances], dtype=float)
        loc_uav_array = np.asarray(loc_uav, dtype=float)
        loc_gt_array = np.asarray(ground_stations, dtype=float)
        observed_positions = self._build_observed_positions(loc_uav_array, loc_gt_array, sensing_distances, sensing_bearings)
    ...
    def step(self, action, state, mode):
        flag = False
        reward = 0.0
        loc_uav = list(self.loc_uav)
        loc_gt = np.array(self.ground_stations, dtype=float)
        self._sample_gt_motion(loc_gt)
        self.ground_stations = np.array(loc_gt, dtype=float)
        prev_loc_uav_array = np.asarray(loc_uav, dtype=float)
        prev_true_distances = np.linalg.norm(prev_loc_uav_array[:2] - loc_gt[:, :2], axis=1)
        prev_centroid = np.mean(loc_gt[:, :2], axis=0)
        ...
        if mode == 2:
            ...
            theta, noisac_assist_heading = self._noisac_assisted_heading(prev_loc_uav_array, loc_gt, theta)
        ...
        self.loc_uav = list(loc_uav)
        loc_uav_array = np.asarray(self.loc_uav, dtype=float)
        bandwidth = self._softmax_bandwidth(action)
        true_distances, sensing_distances, sensing_bearings, fusion_debug, fused_tensor, gate_weights = self._estimate_distances(
            loc_uav_array, loc_gt, mode
        )
        ...
        observed_positions = self._build_observed_positions(
            loc_uav_array, loc_gt, sensing_distances, sensing_bearings
        )
        perception_error = float(
            np.mean(np.linalg.norm(observed_positions[:, :2] - loc_gt[:, :2], axis=1))
            / self.distance_scale
        )
        ...
        throughput_rate = 0.0
        for i, d_true in enumerate(true_distances):
            rate, p_los, _ = self.communication(loc_uav_array[2], d_true, scene=self.scene)
            los_probs[i] = p_los
            throughput_rate += bandwidth[i] * rate
        if mode == 1:
            throughput_rate *= self.large_paper_vlm_rate_scale

        if self.train_gate_online and mode == 3 and fused_tensor is not None:
            gt_tensor = torch.tensor(true_distances, dtype=torch.float32, device=self.device).view(
                self.num_ground_stations, 1
            )
            loss = self.gate_loss_fn(fused_tensor, gt_tensor)
            self.gate_optimizer.zero_grad()
            loss.backward()
            self.gate_optimizer.step()
            fusion_debug["gate_loss"] = float(loss.item())
            if self.train_gate_debug:
                print(f"[Gate] loss={loss.item():.4f}, g={gate_weights.view(-1).detach().cpu().numpy()}")
        ...
        current_centroid = np.mean(loc_gt[:, :2], axis=0)
        ...
        energy_efficiency_reward = (throughput_rate / 1e6) / max(self.E_uav_cost / self.flight_energy_slot(10.4, 0), 1e-6)
        ...
        next_state = self._get_state(self.loc_uav, self.ground_stations, true_distances, sensing_distances, sensing_bearings, los_probs, mode=mode)
        self.prev_heading = theta
        self.prev_horizontal_action = vel_horizontal
        self.prev_vertical_action = vel_vertical

        throughput = throughput_rate * self.t
        energy_efficiency = throughput / max(self.E_uav_cost, 1e-6)
        debug_payload = {
    ...
    def reset(self, k, mode=0):
        ...
        self.prev_vertical_action = None
        loc_uav_array = np.asarray(self.loc_uav, dtype=float)
        loc_gt_array = np.asarray(self.ground_stations, dtype=float)
        true_distances, sensing_distances, sensing_bearings, _, _, _ = self._estimate_distances(loc_uav_array, loc_gt_array, mode)
        los_probs = np.array([self.get_los_probability(self.loc_uav[2], d) for d in true_distances], dtype=float)
        return self._get_state(self.loc_uav, self.ground_stations, true_distances, sensing_distances, sensing_bearings, los_probs, mode=mode)
    ...
    def baseline(self, state):
        action = [-1 for _ in range(self.num_ground_stations + 3)]
        uav_x = state[0] * self.ground_length
        uav_y = state[1] * self.ground_width
        loc_uav_array = [uav_x, uav_y, 100]
        nearest_gs_index = -1
        min_distance = float("inf")
        gt_x = state[3 : 3 + self.num_ground_stations] * self.ground_length
        gt_y = state[3 + self.num_ground_stations : 3 + 2 * self.num_ground_stations] * self.ground_width
        loc_gt_array = np.zeros((self.num_ground_stations, 3), dtype=float)
        for i in range(self.num_ground_stations):
            loc_gt_array[i] = [gt_x[i], gt_y[i], 0.0]
        for i in range(self.num_ground_stations):
            distance = self.index[i] * np.linalg.norm(loc_uav_array[:2] - loc_gt_array[i, :2])
            if distance < min_distance:
                min_distance = distance
                nearest_gs_index = i
        if nearest_gs_index >= 0 and min_distance < 3:
            self.index[nearest_gs_index] = 1000000
        if all(x >= 1000000 for x in self.index):
            action[self.num_ground_stations] = 0
        ...
        action[self.num_ground_stations] = angle / np.pi - 1
        action[self.num_ground_stations + 1] = 1
        action[self.num_ground_stations + 2] = 0
